...
from .app import VormetricService
...
if len(sys.argv) != 2:
    print("Usage: vormetricservice <configuration files directory>")
    sys.exit(1)
...
logging_config_path = os.path.join(config_dir, VormetricService.LOGGING_CONFIG_FILE)
...
with VormetricService(sys.argv[1]) as app:
    try:
        # Run the application
        app.run()
        running = True
